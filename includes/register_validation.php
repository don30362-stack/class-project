...
function validateRegistration(PDO $link, array $post): array
{
    ...
    if ($email === null || $email === '' || strlen($email) > 100 || filter_var($email, FILTER_VALIDATE_EMAIL) === false) return array(false, array('email' => 'Email 格式不正確。'));
    $duplicate = $link->prepare('SELECT 1 FROM member WHERE email = :email LIMIT 1'); $duplicate->execute(array(':email' => $email));
    if ($duplicate->fetchColumn()) return array(false, array('email' => '此 Email 已註冊。'));
    $nameLength = $cname === null ? null : stringLength($cname);
    if ($nameLength === null || $nameLength < 1 || $nameLength > 30) return array(false, array('cname' => '姓名必須為 1～30 個字元。'));
    if ($mobile === null || preg_match('/\A09[0-9]{8}\z/D', $mobile) !== 1) return array(false, array('mobile' => '手機號碼格式不正確。'));
    if ($birthday === null || preg_match('/\A\d{4}-\d{2}-\d{2}\z/D', $birthday) !== 1) return array(false, array('birthday' => '出生日期格式不正確。'));
    $date = DateTimeImmutable::createFromFormat('!Y-m-d', $birthday); $dateErrors = DateTimeImmutable::getLastErrors();
    if (!$date || ($dateErrors !== false && ($dateErrors['warning_count'] > 0 || $dateErrors['error_count'] > 0)) || $date->format('Y-m-d') !== $birthday || $date > new DateTimeImmutable('today')) return array(false, array('birthday' => '出生日期不正確。'));
    $addressLength = $address === null ? null : stringLength($address);
    if ($addressLength === null || $addressLength < 1 || $addressLength > 200) return array(false, array('address' => '地址必須為 1～200 個字元。'));
    if ($city === null || preg_match('/\A[1-9][0-9]*\z/D', $city) !== 1) return array(false, array('myCity' => '請選擇縣市。'));
    if ($town === null || $zip === null || preg_match('/\A[1-9][0-9]*\z/D', $town) !== 1) return array(false, array('myTown' => '請選擇行政區。'));
    ...
    if (!$location->fetchColumn()) return array(false, array('myTown' => '縣市、行政區與郵遞區號不相符。'));
    $imgname = 'avatar.svg';
    if ($upload !== null && $upload !== '') {
        if (!isRegistrationAvatarFilename($upload) || registrationUploadFilename() !== $upload) return array(false, array('uploadname' => '上傳圖片驗證失敗，請重新上傳。'));
        $imgname = $upload;
    }
